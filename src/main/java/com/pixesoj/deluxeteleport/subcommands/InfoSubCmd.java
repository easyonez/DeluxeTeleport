...
public class InfoSubCmd implements SubCommand {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (PlayerUtils.hasPermission(plugin, sender, plugin.getMainPermissionsManager().getInfo(),
                plugin.getMainPermissionsManager().isInfoDefault(), false)) {
            ...
            m.sendMessage(sender, "&bDeluxeTeleport &aby &bPixesoj.", true);
            m.sendMessage(sender, "&7-  &bVersion: &av" + plugin.version, true);
            m.sendMessage(sender, "", true);
        }
        return true;
    }
